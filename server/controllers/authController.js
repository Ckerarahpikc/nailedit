...
const createUser = (user, userType, statusCode, res) => {
  // create token with userType
  const newToken = signToken(user._id, userType);
...
exports.login = catchPromise(async (req, res, next) => {
  ...
  // Update lastLogin for admins
  if (userType === "admin" && user.updateLastLogin) {
    await user.updateLastLogin();
  }

  createUser(user, userType, 200, res);
});
...
exports.register = catchPromise(async (req, res, next) => {
  ...
  // Validate userType
  if (!["client", "admin", "master"].includes(userType)) {
    return next(
      new SetUpError(
...
exports.checkSession = catchPromise(async (req, res, next) => {
  ...
  res.status(200).json({
    ...
    user: {
      ...user.toObject(),
      status: decoded.userType, // Add userType as status for frontend compatibility
    },
  });
});
